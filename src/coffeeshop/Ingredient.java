...
public class Ingredient {
    ...
    public Ingredient(int ingredientId, String name, String description, Unit unit, 
        ...
        this.maximumStock = minimumStock * 10;
    ...
    public void setMinimumStock(double minimumStock) { 
        if (minimumStock >= 0) this.minimumStock = minimumStock; 
    }
    public void setMaximumStock(double maximumStock) { 
        if (maximumStock >= minimumStock) this.maximumStock = maximumStock; 
    }
    public void setCostPerUnit(double costPerUnit) { 
        if (costPerUnit >= 0) this.costPerUnit = costPerUnit; 
    }
    ...
    public double getStockValue() { return currentStock * costPerUnit; }
    public double getStockPercentage() { return maximumStock == 0 ? 0 : (currentStock / maximumStock) * 100; }

    public String getStockStatus() {
        if (isOutOfStock()) return "OUT OF STOCK";
        else if (isLowStock()) return "LOW STOCK";
        else if (currentStock >= maximumStock * 0.8) return "WELL STOCKED";
        else return "NORMAL";
    }
    ...
    public String toString() {
        ...
        if (expirationDate != null) {
            ...
            if (isExpired()) sb.append(" (EXPIRED)");
            else if (isExpiringSoon(7)) sb.append(" (EXPIRES SOON)");
            sb.append("\n");
        }
        if (!supplier.isEmpty()) sb.append(String.format("Supplier: %s\n", supplier));
